#!/usr/bin/env node
/**
 * Convert an ExChek risk triage markdown note (.md) to Word (.docx).
 * Usage: node report-to-docx.mjs <path-to-note.md>
 * Output: <path-to-note.docx> in the same directory.
...
async function main() {
  ...
  if (!mdPath) {
    console.error("Usage: node report-to-docx.mjs <path-to-note.md>");
    process.exit(1);
  }
